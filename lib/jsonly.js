...
var JSONLY = new Class({
	...
	refresh: function(endpoints, next){
		var combined = Array.clone(this.options.endpoints).combine(endpoints);
		if (this.options.endpoints.length !== combined.length) return;
...
//			console.log(JSON.stringify(this.cache));
		console.log('add arrays on index types');

		console.log(this.options.endpoints);
...
//			var data = endpoint[type];
			if (this.cache[endpoint].meta.type == 'index') {
				console.log('the ' +endpoint+ ' needs a collection property');
//				this.cache[endpoint].index.for
